...
const login = async (req, res) => {
  ...
  try {
    const [user] = await DBConnection.query(
      "SELECT username, userid, password, role_id FROM users WHERE email = ?",
      [email]
    )
    ...
    const token = jwt.sign({ username, userid, role_id }, process.env.JWT_SECRET, {
      expiresIn: "1d",
    })
    ...
    return res
      .status(200)
      .json({ msg: "User login successful", token, username, userid, role_id })
  } catch (error) {
